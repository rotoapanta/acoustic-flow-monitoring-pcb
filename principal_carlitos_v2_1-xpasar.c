...
static void interrupt isr(void)
{
	...
	{   
		...
		if(TAREA_ADQ<=CONT_TAREA_ADQ)			//operar cada 0.7 segundo
		 {
			...
			for(i=0;i<50;i++)
			{
				...
				dato_spi_temp=0;
				dato_spi_temp =SPI_Recibir_Caracter()<<8;
		...
		if(VERIFICAR_UMBRAL)
		{
			...
			{
				CONT_TIEMPO_UMBRAL++;
				if(TIEMPO_UMBRAL<=CONT_TIEMPO_UMBRAL)   // TIEMPO_UMBRAL=30, fija 0.4s
				{
